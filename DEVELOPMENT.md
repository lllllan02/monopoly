...
### 2.2 地图设计 (Map Design)
专注于地图的拓扑结构与逻辑编排，实现“可视化拖拽”设计：
*   **多地图支持**：每个主题下可创建多个不同规模（格数）的地图。
*   **可视化编辑器**：
    - **地块备选库**：左侧展示当前主题下所有已创建的地块模型（包含系统内置与自定义地块）。
    - **拖拽配置 (Drag & Drop)**：将地块从备选库直接拖入对应的棋盘槽位。
    - **插槽属性管理 (Slot Properties)**：支持定义格位的逻辑类型（如普通地块、起点、税收格等）并编辑格位名称。
*   **地图预览**：编辑器内可直观查看到地图格位的顺序编号与关联的地块名称。
*   **数据结构**：
    - `Map`: { `id`, `name`, `themeId`, `size`, `slots: MapSlot[]` }
    - `MapSlot`: { `type`, `propertyId?`, `name?`, `value?` }
